function [episodeVector, onsetVector, offsetVector] = findPepisodesOnsetOffset(powerVal, powerThresh, durationThresh, frequency, samplingRate)

% [episodeVector, onsetVector, offsetVector] = findPepisodesOnsetOffset(powerVal, powerThresh, durationThresh, frequency, samplingRate)
...
% threshold and duration threshold. Will also return vectors that indicate
% when the oscillation(s) started and stopped.
%
% INPUTS:
...
%
% OUTPUT:
%   episodeVector: binary vector that indicates for each timepoint whether a
%       sustained oscillation was present (1) or not (0)
%   onsetVector: binary vector that indicates for each timepoint whether an
%       oscillation was initiated (1) or not (0)
%   offsetVector: binary vector that indicates for each timepoint whether
%       an oscillation stopped (1) or not (0)
...
episodeVector = zeros(1, length(powerVal));
onsetVector   = episodeVector;
offsetVector  = episodeVector;
...
if (~isempty(aboveThreshInterval)) 
    ...
    % Make the binary vectors for output
    for thisGoodEpoch = 1:size(aboveThreshInterval, 2)
        episodeVector(aboveThreshInterval(1, thisGoodEpoch):aboveThreshInterval(2, thisGoodEpoch)) = 1;
        onsetVector(aboveThreshInterval(1, thisGoodEpoch)) = 1;
        offsetVector(aboveThreshInterval(2, thisGoodEpoch)) = 1;
    end
end
