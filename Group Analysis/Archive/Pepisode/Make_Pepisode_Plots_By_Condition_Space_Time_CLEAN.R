...
for (thisSubject in 1:length(subjectIDs)) {
  ...
  for (thisTeleporter in 1:length(teleporters)) {
    ...
    csvFile <- paste0(expDir, subjectIDs[thisSubject], "/Mat Files/Pepisode/Summary/", subjectIDs[thisSubject], "_", teleporters[thisTeleporter], "_pepisode_summary_noSpikes_noWaves.csv")
...
write.csv(allPepisodeData, file = "All_Subjects_Pepisode_21May2015_CLEAN.csv", row.names = FALSE)
...
ggsave('Figures/Pepisode_by_timepoint_normSEM_21May2015.png')
...
ggsave('Figures/Pepisode_by_space_normSEM_21May2015.png')
...
ggsave('Figures/Pepisode_by_Time_normSEM_21May2015.png')
