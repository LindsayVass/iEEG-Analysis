...
for (thisSubject in 1:length(subjectIDs)) {
  ...
  for (thisTeleporter in 1:length(teleporters)) {
    ...
    csvFile <- paste0(expDir, subjectIDs[thisSubject], "/Mat Files/Pepisode/Summary/", subjectIDs[thisSubject], "_", teleporters[thisTeleporter], "_pepisode_summary_noSpikes_noWaves_3sBuffer.csv")
...
saveFile <- paste0("All_Subjects_Pepisode_CLEAN_3sBuffer", today, ".Rda")
...
ggsave(paste0('Figures/Pepisode_by_timepoint_normSEM_3sBuffer', today, '.png'))


# make a plot showing pepisode by timepoint for each electrode
meanByElectrode <- allPepisodeData %>%
  filter(is.na(Pepisode) == FALSE) %>%
  select(TimePoint, Pepisode:FrequencyBand) %>%
  group_by(ElectrodeID, TimePoint, FrequencyBand) 
thetaByElectrode <- meanByElectrode %>%
  filter(FrequencyBand == "Theta") %>%
  summarise(PepisodeMean = mean(Pepisode), SEM = sd(Pepisode)/sqrt(n()))
ggplot(thetaByElectrode, aes(x = TimePoint, y = PepisodeMean, ymin = PepisodeMean - SEM, ymax = PepisodeMean + SEM)) +
  geom_point(size = 4) +
  geom_pointrange() + 
  facet_wrap(~ElectrodeID, scales = "free") +
  ggtitle("Theta Pepisode by Electrode")
...
ggplot(meanBySpace, aes(x = TimePoint, y = Pepisode, ymin = Pepisode - NormSEM, ymax = Pepisode + NormSEM, color = TrialSpaceType)) +
  ...
  geom_pointrange() +
  facet_wrap(~ FrequencyBand, scales = "free") + 
  ggtitle("Pepisode by Space Condition")

ggsave(paste0('Figures/Pepisode_by_space_normSEM_3sBuffer', today, '.png'))
...
ggplot(meanByTime, aes(x = TimePoint, y = Pepisode, ymin = Pepisode - NormSEM, ymax = Pepisode + NormSEM, color = TrialTimeType)) +
  ...
  geom_pointrange() +
  facet_wrap(~ FrequencyBand, scales = "free") + 
  ggtitle("Pepisode by Time Condition")

ggsave(paste0('Figures/Pepisode_by_time_normSEM_3sBuffer', today, '.png'))
