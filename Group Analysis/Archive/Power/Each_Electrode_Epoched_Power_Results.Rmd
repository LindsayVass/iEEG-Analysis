---
title: "Histograms of Epoched Power Differences within Electrodes"
author: Lindsay Vass
date: 29 May 2015
...
Each trial was first epoched into contiguous time bins that surrounded the 
teleportation event. These bins were defined as follows (t0 = time of teleporter 
entry):
...
I then added a buffer to the beginning and end of each epoch. This buffer was
equal to the length of 3 cycles at the lowest frequency. In this dataset, that
was 3 cycles * `r min(allPowerData$Frequency)` Hz = `r 3 * min(allPowerData$Frequency) * 1000`
 ms. Six-cycle wavelets were then used to estimate the power at each frequency
and time point, at which point the buffers were trimmed from the data. I then 
took the log(Power) and z-scored these values within each electrode and 
frequency. All plotted values are of these z-scored log(Power) values.
...
for each trial to examine whether there were significant power differences
between spatial conditions across trials. I used a Mann-Whitney U test to 
evaluate the significance of the following two comparisons:
...
for each trial to examine whether there were significant power differences
between temporal conditions across trials. I used a Mann-Whitney U test to 
evaluatethe significance of the following two comparisons:
