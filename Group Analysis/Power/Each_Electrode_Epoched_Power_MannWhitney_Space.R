# Script Name:  Each_Electrode_Epoched_Power_MannWhitney_Space.R
# Author:       Lindsay Vass
# Date:         29 May 2015
# Purpose:      This script will test whether power significantly differs 
#               between temporal conditions (NS/FS) within each frequency band 
...
#               This script differs from "Each_Electrode_Power_MannWhitney_Space"
...
allPowerData <- allPowerData %>%
  ...
  select(TrialNumber, TrialSpaceType, TimePoint, Power:FrequencyBand) 
...
for (thisElectrode in 1:nlevels(allPowerData$ElectrodeID)){
  ...
  for (thisFreqBand in 1:nlevels(allPowerData$FrequencyBand)) {
    ...
    frequencyData <- electrodeData %>%
      ...
      group_by(TrialNumber, TrialSpaceType) %>%
      summarise(Power = mean(Power))
    
    # Cast to wide format
    frequencyDataWide <- frequencyData %>%
      dcast(TrialNumber ~ TrialSpaceType, value.var = "Power")
...
# Save the chart
today <- Sys.Date()
ggsave(filename = paste0("Figures/Each_Electrode_Epoched_Power_MannWhitney_Space_Bar_", today, ".png"))
...
# between NS & FS within each timepoint.
...
for (thisElectrode in 1:nlevels(allPowerData$ElectrodeID)){
  ...
  for (thisFreqBand in 1:nlevels(sigFreqBands)) {
    ...
    for (thisTimePoint in 1:nlevels(allPowerData$TimePoint)) {
      ...
      NSGtFS  <- wilcox.test(timePointDataWide$NS, timePointDataWide$FS, alternative = "greater")
      NSLtFS  <- wilcox.test(timePointDataWide$NS, timePointDataWide$FS, alternative = "less")
...
# # Save the chart
# today <- Sys.Date()
# ggsave(filename = paste0("Figures/Each_Electrode_Epoched_Power_MannWhitney_Space_FollowUp_By_TimePoint_Bar_", today, ".png"))
# 
