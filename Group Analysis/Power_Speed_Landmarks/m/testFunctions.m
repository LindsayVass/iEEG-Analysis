...
assert(length(epochOnsets) == length(systemTimeStart), 'Converting ticks to EEG indices changed the number of time points.')
...
[epochOnsets, EDF1inds, EDF2inds, missingInds] = getEpochOnsets(pulseTimingPaths, systemTimeStart);

assert(size(epochOnsets{1}, 2) + size(epochOnsets{2}, 2) + length(missingInds) == length(systemTimeStart), 'Converting ticks to EEG indices changed the number of time points.')
