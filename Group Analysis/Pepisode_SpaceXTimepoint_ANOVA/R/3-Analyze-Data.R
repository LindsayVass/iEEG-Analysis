...
#               Space (NS/FS) and Timepoint (Pre/Tele/Post)
...
for (thisFreqBand in 1:nlevels(cleanData$FrequencyBand)) {
  ...
  for (thisTimeType in 1:nlevels(cleanData$TrialTimeType)) {
    ...
    tempResults <- data.frame(FreqBand = as.character(levels(cleanData$FrequencyBand)[thisFreqBand]),
                            TimeType = as.character(levels(cleanData$TrialTimeType)[thisTimeType]),
...
# save data
save(file = 'Rda/allAnalyzedData.Rda', 'anovaResults')
