# Script Name:  Each_Electrode_Epoched_EqualTimeBins_Pepisode_Wilcoxon_by_Timepoint.R
# Author:       Lindsay Vass
# Date:         2 June 2015
...
#               a bar graph. Data were epoched prior to calculating pepisode. In
#               this analysis, the within-trial time bins are equal in length
#               (i.e., Pre = Tele = Post)
...
for (thisSubject in 1:length(subjectIDs)) {
  ...
  for (thisTeleporter in 1:length(teleporters)) {
    ...
    csvFile <- paste0(expDir, subjectIDs[thisSubject], "/Mat Files/Pepisode/Summary/", subjectIDs[thisSubject], "_", teleporters[thisTeleporter], "_epoched_pepisode_equalTimeBins_summary_noSpikes_noWaves_3sBuffer.csv")
...
today <- Sys.Date()
saveFile <- paste0("All_Subjects_Epoched_Pepisode_EqualTimeBins_CLEAN_3sBuffer_", today, ".Rda")
...
# Save the chart
today <- Sys.Date()
ggsave(filename = paste0("Figures/Each_Electrode_Epoched_Pepisode_EqualTimeBins_Wilcoxon_by_Timepoint_Bar_", today, ".png"))
...
for (thisFreqBand in 1:nlevels(allPepisodeData$FrequencyBand)) {
  ...
  # Save the chart
  today <- Sys.Date()
  ggsave(filename = paste0("Figures/Each_Electrode_Epoched_", levels(allPepisodeData$FrequencyBand)[thisFreqBand], "_Pepisode_EqualTimeBins_by_Timepoint_", today, ".png"))
