...
pepisodeSummary      = cell(1,5);
pepisodeSummary(1,:) = {'SubjectID','Teleporter','Electrode','Frequency','Pepisode'};
powerSummary         = cell(1,5);
powerSummary(1,:)    = {'SubjectID', 'Teleporter', 'Electrode','Frequency', 'Power'};
thisRow = 2;
...
for thisDepth = 1:length(depthNames)
    ...
    for thisChan = 1:length(chanNames)
        ...
        [~, percentTimePepisode, meanPower] = calcPepisodeWholeExpt(powerDistribution, frequencies, eegList, chanNames{thisChan}, 95, 3);
