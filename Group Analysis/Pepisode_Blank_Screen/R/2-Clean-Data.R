...
blankPepisode <- blankData %>%
  ...
  filter(Frequency < 8)
blankPepisode$Condition <- plyr::mapvalues(blankPepisode$Condition, from = c('FreeExplore', 'Navigation'), to = c('Blank', 'Blank'))
...
save(file = 'Rda/allCleanData.Rda', list = 'allPepisode')