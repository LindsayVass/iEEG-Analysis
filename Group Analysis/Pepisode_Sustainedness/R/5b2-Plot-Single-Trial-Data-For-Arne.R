...
#dir.create('Figures/RawTracePlusPepisodeAndPSD/')
...
plotSubsTrials <- manualBestTrials %>%
...
for (i in 1:nlevels(allData$ObservationID)) {
  ...
  freqInterval <- floor(length(freqList) / 5)
  freqInds <- seq(1, length(freqList), freqInterval)
