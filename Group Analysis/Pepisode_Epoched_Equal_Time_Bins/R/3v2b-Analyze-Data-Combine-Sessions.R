# Script Name:  3v2b-Analyze-Data-Combine-Sessions.R
# Author:       Lindsay Vass
...
#               significantly differs between timepoints. For electrodes that 
#               two recording sessions, the data will be combined.
...
getCorrectedP <- function(permData, trueData, trueOrPerm = "true") {
  ...
  trueDataCorrected <- trueDataCorrected %>%
    ungroup() %>%
    group_by(Electrode, FrequencyBand, Contrast) 
  ...
  if (tolower(trueOrPerm) == "perm") {
    trueDataGt <- trueData %>%
      ...
      inner_join(trueDataCorrectedGt, by = c("Electrode", "FrequencyBand", "statistic", "p.value", "Contrast", "Iteration")) %>%
      select(-Iteration) %>%
      inner_join(origData, by = c("Electrode", "FrequencyBand", "statistic", "p.value", "Contrast"))
    trueDataLt <- trueData %>%
      filter(Contrast == "Pre < Tele" | Contrast == "Tele < Post") %>%
      inner_join(trueDataCorrectedLt, by = c("Electrode", "FrequencyBand", "statistic", "p.value", "Contrast", "Iteration")) %>%
      select(-Iteration) %>%
      inner_join(origData, by = c("Electrode", "FrequencyBand", "statistic", "p.value", "Contrast"))
  } else {
    trueDataGt <- trueData %>%
...
cleanData <- tidyr::separate(cleanData, ElectrodeID, c('Subject', 'Session', 'Electrode'), sep = '_')
cleanData <- cleanData %>%
  mutate(Electrode = paste(Subject, Electrode, sep = "_"))

cleanData <- cleanData %>%
  group_by(Electrode, FrequencyBand, TrialNumber, TimePoint) %>%
  summarise(MeanPepisode = mean(Pepisode)) 

trueData <- cleanData %>%
  group_by(Electrode, FrequencyBand, TrialNumber, TimePoint) %>%
  dcast(TrialNumber + Electrode + FrequencyBand ~ TimePoint, value.var = "MeanPepisode") %>%
  group_by(Electrode, FrequencyBand) %>%
...
moltenP <- trueData %>%
  ...
  melt(id.vars = c("Electrode", "FrequencyBand"), variable.name = "Contrast", value.name = "PValue")
moltenP$Contrast <- sub("_pvalue", "", moltenP$Contrast)
moltenZ <- trueData %>%
  select(-c(TeleLtPre, TeleGtPre, TeleGtPost, TeleLtPost)) %>%
  select(-(ends_with("pvalue"))) %>%
  melt(id.vars = c("Electrode", "FrequencyBand"), variable.name = "Contrast", value.name = "Statistic")
...
permNSigElectrodes <- permData %>%
  melt(id.vars = c("Electrode", "FrequencyBand", "Iteration"), variable.name = "Contrast", value.name = "PValue") %>%
...
save(file = 'Rda/allAnalyzedData_Combine_Sessions.Rda', list = c('cleanData', 'electrodePCorr', 'moltenTrueData', 'permData', 'permNSigElectrodes', 'trueData', 'trueNSigElectrodes'))