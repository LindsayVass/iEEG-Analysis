...
sampleData <- function(thisData, trainingSize, testingSize) {
  ...
  allTestTrials$TrialSpaceType <- factor(allTestTrials$TrialSpaceType, levels = c('NS', 'FS'))
...
runClassification <- function(classData, thisResult, numPerm = 1000, trainingPercent = 0.75) {
  ...
  for (i in 1:numPerm) {
    ...
    confusionMatrix <- table(predictions, thisSample$testTrials$TrialSpaceType)
    accuracy <- sum(diag(confusionMatrix)) / sum(confusionMatrix)
    accuracyResults <- rbind(accuracyResults, accuracy)
  ...
  accuracyResults <- mean(accuracyResults[2:1001,1])
...
# Prep Raw Data ----------------------------------------------------------------
load('Rda/pepisodeTrials.Rda')
rawData <- readMat('mat/OccipRawDataPepisodeTrials_1-8Hz.mat')
bestData <- occipData
...
topClass <- occipitalResults %>%
  ungroup() %>%
  arrange(desc(Accuracy))
topClass <- topClass %>%
  select(ElectrodeID, Model, CorrP) %>%
...
for (thisElec in 1:nlevels(bestData$ElectrodeID)) {
  ...
  minLength <- min(c(length(nsInd), length(fsInd)))
  
  for (thisTrialPair in 1:minLength) {
    nsBinary <- t(as.data.frame(allBinaryData[nsInd[thisTrialPair]])) %>%
      as.data.frame()
    fsBinary <- t(as.data.frame(allBinaryData[fsInd[thisTrialPair]])) %>%
      as.data.frame()
    ...
    plotColorFreq <- round(bestData$Frequency[nsInd[thisTrialPair]], digits = 2)
    plotColorInd  <- which(names(nsBinary) == plotColorFreq)
    
    allEEG <- data.frame(TrialSpaceType = c(rep('NS', length(unlist(allEEGData[nsInd[thisTrialPair]]))), rep('FS', length(unlist(allEEGData[fsInd[thisTrialPair]])))),
                         EEG = c(unlist(allEEGData[nsInd[thisTrialPair]]), unlist(allEEGData[fsInd[thisTrialPair]])),
    ...
    fileName <- paste0('Figures/Single Electrode/Occipital/', electrode, '_', thisTrialPair, '.pdf')
