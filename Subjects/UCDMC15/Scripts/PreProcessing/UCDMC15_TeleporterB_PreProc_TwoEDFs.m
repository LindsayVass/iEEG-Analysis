...
edf_file_1  = [subject_dir 'Raw Data/UCDMC15_04_02_15_teleporter.edf'];
edf_file_2  = [subject_dir 'Raw Data/UCDMC15_04_02_15_teleporter1.edf'];

% txt files output by Unity
unityPulsesFile = [subject_dir 'Behavioral Data/TeleporterB/s3_PULSES_.txt'];
unityDataPath   = [subject_dir 'Behavioral Data/TeleporterB/s3_FindStore_TeleporterB_FIXED.txt'];
...
if redoEpochs == 1
    ...
    for thisTrial = 1:length(teleporterInds)
        ...
        if (onsetTick - unityTicks1(end) < 0) % if trial in EDF1
            ...
            fitInds(fitInds > length(unityTicks1)) = [];
        ...
        elseif (onsetTick - unityTicks2(1) < 0) % trial in lost EEG between EDF files
            ...
            % get fit of line using the inds selected above
            fitInds(fitInds > length(unityTicks2)) = [];
            fitInds(fitInds < 1) = [];
