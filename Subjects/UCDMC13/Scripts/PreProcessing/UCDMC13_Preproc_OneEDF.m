...
%% Set up paths
subjectDir = '/Users/Lindsay/Documents/MATLAB/iEEG/Subjects/UCDMC13/';
subjectID  = 'UCDMC13';
teleporter  = 'TeleporterA';
edfFile    = 'UCDMC13_112114.edf'; % in 'Raw Data' folder

% Psychtoolbox data
preTestPulses  = [subjectDir 'Behavioral Data/Marker Pre-Testing/SubjectUCDMC13_112114_Data.mat'];
postTestPulses = [subjectDir 'Behavioral Data/Marker Post-Testing/SubjectUCDMC13_112114_post_ret_Data.mat'];

% Unity data
unityFindStorePath = [subjectDir 'Behavioral Data/s1_patientTeleporterData.txt'];
...
%% Fix marker channel
% Fix marker channel. For some reason, the marker channel
% for this subject only recorded 1 sample per every 32 of the other
% channels. In other words, it was collected at 16 Hz whereas the rest of
% the data was collected at 512 Hz. We're basically going to find all the
% pulses ourselves and create a new marker channel based on what we found. 
...
leading_zeros = 31; % number of zeros to insert before each marker channel data point (consult the ASCII file)
old_mc_orig = EEG.data(markerChanNumber,:);
old_mc = old_mc_orig(find(old_mc_orig));

new_mc = [];
ind = 1;
zeroPad = zeros(31,1);
while length(new_mc) < size(EEG.data,2)
    
    new_mc = cat(1, new_mc, zeroPad, old_mc_orig(ind));
...
for thisDepth = 1:length(depthNames)
    ...
    save(['Mat Files/' subjectID '_' teleporter '_' depthNames{thisDepth} '_noSpikes_noWaves_unepoched_goodEpochs.mat'], 'goodNoWaveEpochs');
end