...
subject_id  = 'UCDMC15';
edf_file    = 'UCDMC15_04_01_15_teleporter1.edf'; % in 'Raw Data' folder
save_stem   = 'UCDMC15_TeleporterA_EDF2';
...
badChans = input('Which channel(s) should be removed? If multiple, enclose in brackets []: ');
EEG.data(badChans,:) = [];
...
if size(EEG.data, 2) / EEG.srate >= lastPulseSec + bufferSec % don't trim unless the post-pulse EEG length exceeds the buffer
...
% Plot time around the last pulse
plotBufferBin = 250;
markerData    = data(markerChanNumber,:);
h = figure;
ax = gca;
plot(markerData(lastPulseBin:lastPulseBin-(plotBufferBin*2)));
title('Last pulse?');
...
while 1
    ...
    lastPulseBin = lastPulseBin - plotBufferBin*2;
    numTries = numTries + 1;
    
    plot(ax, markerData(lastPulseBin-plotBufferBin:lastPulseBin+plotBufferBin));
    title('Last pulse?');
    answer = questdlg(prompt);
...
if (numTries == 1)
    plotRange = markerData(lastPulseBin:lastPulseBin - (plotBufferBin * 2));
else
    plotRange = markerData(lastPulseBin-plotBufferBin:lastPulseBin+plotBufferBin);
end
...
if(strcmpi(answer,'Yes') == 1)
    if(strcmpi(pulsePolarity,'Positive') == 1)
        ...
        lastPulseBin = lastPulseBin - plotBufferBin + maxValInd;
        close(h);
        
    else
        lastPulseBin = lastPulseBin - plotBufferBin + minValInd;
        close(h)
    end
else
...
pulsesBin   = round(pulsesRef/ticksPerBin) + lastPulseBin;

% Ask for visual confirmation of each pulse
prompt = 'Does the plot show a pulse with a dotted red line at the peak?';
pulsesBinConf = lastPulseBin;
currBin = lastPulseBin - EEG.srate;

 pulseNum = 2;
% pulse #451 had strange morphology
% pulse #838
% pulse #1747
while 1
    h = figure;
    ...
    % Plot pulse with peak indicated by a red dashed line. If the peak of
    % the pulse was incorrectly selected and manual intervention was
    % needed, restart executing the script from here.
    plot(ax, x, y);
    yLims = get(ax,'ylim');
    hold on;
...
keyboard; % in the event that a pulse has an issue (delayed or multiple peaks), this prevents the script from continuing on
...
unityTicks = pulses(length(pulses)-length(indEEG)+1:end);
