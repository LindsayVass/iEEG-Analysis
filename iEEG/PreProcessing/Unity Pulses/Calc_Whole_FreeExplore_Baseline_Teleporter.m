...
unepochedEEG     = [subject_dir 'Raw Data/UCDMC15_TeleporterA_EDF1_unepoched.set'];
unityFile        = [subject_dir 'Behavioral Data/TeleporterA/s3_FreeExplore_TeleporterA.txt'];
saveFile         = [subject_dir 'Mat Files/UCDMC15_TeleporterA_FreeExplore_baseline.mat']; % mat file with mean power at each electrode/frequency
epochEEGSaveFile = [subject_dir 'Epoched Data/UCDMC15_TeleporterA_FreeExplore_baseline_epoched.mat']; % EEG file of the epoched free exploration data
timeSyncFile     = [subject_dir 'Raw Data/UCDMC15_TeleporterA_EDF1_pulse_timing.mat'];
...
%% convert start time from ticks to EEG bins
load(timeSyncFile);

 % find the pulse time (in ticks) closest to our start tick
 tickDiff   = abs(startTicks - unityTicks);
 ...
 startBin = round(startTicks*fit_P(1) + fit_P(2));
 
 h = figure;
 plot(unityTicks(fitInds),indEEG(fitInds),'k*')
 hold on;
 plot(unityTicks(fitInds),fit_y,'-')
 scatter(startTicks,startBin,'ro')
...
%% convert end time from ticks to EEG bins

 % find the pulse time (in ticks) closest to our start tick
 tickDiff   = abs(endTicks - unityTicks);
 minDiffInd = find(tickDiff == min(tickDiff));
 ...
 fit_P = polyfit(unityTicks(fitInds),indEEG(fitInds),1);
 fit_y = polyval(fit_P,unityTicks(fitInds));
 endBin = round(endTicks*fit_P(1) + fit_P(2));
 
 h = figure;
 plot(unityTicks(fitInds),indEEG(fitInds),'k*')
 hold on;
 plot(unityTicks(fitInds),fit_y,'-')
 scatter(endTicks,endBin,'ro')
...
%% get start times for each epoch
