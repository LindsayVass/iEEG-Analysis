function calcEpochedPepisodeLKV(EEG, chanNames, trialTypes, epochOnsets, epochOffsets, saveStem, frequencies, width, bufferCycles, amplitudeThresh)
% calcEpochedPepisodeLKV: Calculate power on the clean segments of the EEG.
...
%
%   amplitudeThresh: percent of baseline distribution to use as amplitude
%       threshold for oscillations (suggested = 95)
%
...
for thisChan = 1:length(chanNames)
    ...
    % Hold data for this channel so we can zscore it at the end
    chanData = allPowerData;
    ...
    % Concatenate all the data so we can zscore it for each frequency
    allChanData = horzcat(chanData{:,:});
    allChanDataZ = zscore(allChanData, 0, 2);
    
    % Get the data back out
    colInd = 1;
    for thisCol = 1:size(chanData, 2)
        
        for thisRow = 1:size(chanData, 1)
            
            expectedCols = size(chanData{thisRow, thisCol}, 2);
            allPowerData{thisRow, thisCol} = allChanDataZ(:, colInd:colInd + expectedCols - 1) ;
            colInd = colInd + expectedCols;
